...
interface InventoryProps {
  ...
  onUploadImage: (id: number, name: string, imageUrl: string | null) => void;
  addForm: {
    name: string;
    setName: any;
    price: string;
    ...
    <div className="animate-fade-up duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {products.map((product) => (
          <div
            key={product.id}
            className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all text-left group overflow-hidden"
          >
            {/* รูปภาพสินค้า */}
            <div
              className="relative w-full bg-slate-100 cursor-pointer overflow-hidden"
              style={{ height: "160px" }}
              onClick={() => onUploadImage(product.id, product.name, product.image_url)}
            >
              {product.image_url ? (
                <>
                  <img
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                    <span className="text-white text-sm font-bold opacity-0 group-hover:opacity-100 transition-all bg-black/50 px-3 py-1.5 rounded-xl">
                      🖼️ เปลี่ยนรูป
                    </span>
                  </div>
                </>
              ) : (
                <div className="w-full h-full flex flex-col items-center justify-center gap-1 text-slate-300 hover:text-slate-400 hover:bg-slate-200 transition-all">
            ...
            <div className="p-5">
              <div className="flex justify-between mb-3 font-bold">
                <h2 className="text-xl text-slate-800 group-hover:text-blue-700 transition-colors truncate pr-2">
                  {product.name}
                </h2>
                <div className="flex gap-1 shrink-0">
                  <button
                    className="hover:bg-blue-50 p-1.5 rounded-lg transition-colors"
                    onClick={() => onUpdatePrice(product.id, product.price, product.name)}
                  >
                    ✏️
                  </button>
                  <button
                    className="hover:bg-red-50 p-1.5 rounded-lg transition-colors"
              ...
              <div className="flex items-center justify-between bg-slate-50 p-3 rounded-2xl border border-dashed border-slate-200">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`text-xl font-black min-w-[30px] text-center ${
                      product.stock < 5 ? "text-red-600 animate-pulse" : "text-slate-800"
                    }`}
                  >
                    {product.stock}
                  </span>
